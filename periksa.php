                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="card card-primary">
                                    <div class="card-header">
                                        <h3 class="card-title">Masukan Tindakan</h3>
                                    </div>
                                    <form action="input_periksa.php" method="post">
                                        <div class="card-body">
                                            <div class="form-group">
                                                ...
                                                <?php
                                                $sql = mysqli_query($conn, "SELECT id_trans FROM trans_tindakan");
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="nama_tindakan" id="">
                                                    <option value="">--Pilih--</option>
                                                    <?php
                                                    $sql1 = mysqli_query($conn, "SELECT * FROM mast_tindakan");
                                                    while ($dt = mysqli_fetch_array($sql1)) {

                                                    ?>
                                                        <option value="<?php echo $dt['nama_tindakan']; ?> "><?php echo $dt['nama_tindakan']; ?></option>
                            ...
                            <div class="col-md-6">
                                <div class="card card-success">
                                    <div class="card-header">
                                        <h3 class="card-title">Daftar Pemeriksaaan Pasien</h3>
                                    </div>
                                    <div class="card-body">
                                        <table class="table table-bordered">
                                            ...
                                            <form action="simpan_hasil.php" method="post">
                                                <tbody>
                                                    ...
                                                    $no = 1;
                                                    $no_rm = $_GET['no_rm'];
                                                    $dta_periksa = mysqli_query($conn, "SELECT * FROM trans_tindakan WHERE no_rm = $no_rm");
                                                    while ($d = mysqli_fetch_array($dta_periksa)) {
                                                    ?>
                                                        <tr>
                                                            <td name="no_rm" value="" hidden><?php echo $d['no_rm']; ?></td>
                                                            <td><?php echo $no++; ?></td>
                                                            <td name="nama_tindakan"><?php echo $d['nama_tindakan']; ?></td>
                                                            <td name="harga"><?php echo $d['harga']; ?></td>
                                                            <td>
                                                                <a href="delete_tindakan.php?id_trans=<?= $d['id_trans']; ?>&no_rm=<?= $d['no_rm']; ?>" class="btn btn-danger" onclick="return confirm('Yakin Mau Hapus Tindakan ?');">X</a>
                                                            </td>
                                                ...
                                                <tfoot>
                                                    <?php
                                                    $q = mysqli_query($conn, "SELECT SUM(harga) as total FROM trans_tindakan WHERE no_rm = $no_rm");
